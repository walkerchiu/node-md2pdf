...
export class CustomizationMode {
      ...
      while (running) {
        ...
        switch (option) {
          ...
          case 'metadata':
            ...
            break;
          case 'password':
            await this.passwordProtectionSettings();
            break;
          case 'watermarks':
  ...
  private async passwordProtectionSettings(): Promise<void> {
    while (true) {
      console.clear();
      ...
      const option = await this.selectPasswordProtectionOption();

      switch (option) {
        case 'preview':
          await this.previewPasswordProtectionSettings();
          break;
        case 'toggle-protection':
          await this.togglePasswordProtection();
          break;
        ...
        case 'configure-permissions':
          ...
          break;
        case 'reset-defaults':
          await this.resetPasswordProtectionDefaults();
          break;
        case 'back':
          return;
        default:
  ...
  private async selectPasswordProtectionOption(): Promise<string> {
    const inquirer = await import('inquirer');
   ...
   */
  private displaySecuritySubHeader(
    subFeature:
      | 'preview'
      | 'userPassword'
      | 'ownerPassword'
  ...
  private async previewPasswordProtectionSettings(): Promise<void> {
    console.clear();

    // Display sub-feature header
    this.displaySecuritySubHeader('preview');
  ...
  private async togglePasswordProtection(): Promise<void> {
    console.clear();

    // Display sub-feature header
    this.displaySecuritySubHeader('toggleProtection');
  ...
  private async setUserPassword(): Promise<void> {
    ...
    // Display sub-feature header
    this.displaySecuritySubHeader('userPassword');
  ...
  private async setOwnerPassword(): Promise<void> {
    ...
    // Display sub-feature header
    this.displaySecuritySubHeader('ownerPassword');
  ...
  private async resetPasswordProtectionDefaults(): Promise<void> {
    console.clear();

    // Display sub-feature header
    this.displaySecuritySubHeader('resetDefaults');
  ...
  private async configurePermissions(): Promise<void> {
    ...
    while (true) {
      ...
      this.displaySecuritySubHeader('configurePermissions');
  ...
  private async templateManagement(): Promise<void> {
    ...
    while (running) {
      this.showTemplateManagementHeader();
      const option = await this.selectTemplateManagementOption();
   ...
   */
  private showTemplateManagementHeader(): void {
    console.clear();
   ...
   */
  private async selectTemplateManagementOption(): Promise<string> {
    const inquirer = await import('inquirer');
    ...
    try {
      ...
      if (collection.system.length === 0) {
        ...
        collection.system.forEach((template, index) => {
          ...
          console.log(); // Add spacing before description
          console.log(
            chalk.gray(
              `     ${this.translationManager.t(template.description)}`,
            ),
          );
          console.log(); // Add spacing after description
  ...
  private async createNewTemplate(): Promise<void> {
    ...
    try {
      ...
      const { id, ...templateInput } = templateToValidate;
   ...
   */
  private translateValidationError(message: string): string {
    // Map English error messages to translation keys
    const errorMap: Record<string, string> = {
      'Template ID is required and must be a string':
        'templates.validation.errors.idRequired',
      'Template name is required and must be a string':
        'templates.validation.errors.nameRequiredString',
      'Template description is required and must be a string':
        'templates.validation.errors.descriptionRequiredString',
      'Template type must be either "system" or "custom"':
        'templates.validation.errors.typeInvalid',
    };

    const translationKey = errorMap[message];
    return translationKey ? this.translationManager.t(translationKey) : message;
  }
  ...
  private getPresetPermissions(preset: string): {
    printing: boolean;
    modifying: boolean;
    copying: boolean;
    annotating: boolean;
  ...
  private async savePermissions(permissions: any): Promise<void> {
    const currentSettings = this.configManager.get('passwordProtection', {
      enabled: false,
    });
