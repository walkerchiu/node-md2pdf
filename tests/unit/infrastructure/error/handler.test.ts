...
describe('ErrorHandler', () => {
  ...
  test('categorizeError recognizes MD2PDFError and common messages', () => {
    const h = new ErrorHandler();
    expect(h.categorizeError(new MD2PDFError('x', 'C', 'file_system', false))).toBe('file_system');
    expect(h.categorizeError(new Error('ENOENT: something'))).toBe('file_system');
    expect(h.categorizeError(new Error('permission denied'))).toBe('file_system');
    expect(h.categorizeError(new Error('JSON parse failed'))).toBe('configuration');
  ...
  test('getSuggestions returns tailored suggestions for known errors', () => {
    ...
    expect(h.getSuggestions(new FilePermissionError('/tmp/x', 'read'))[0]).toContain(
      'Check file permissions'
    );
    expect(h.getSuggestions(new MarkdownParsingError('oops'))[0]).toContain(
      'Check the Markdown syntax'
    );
    expect(h.getSuggestions(new PDFGenerationError('boom', 'render'))[0]).toContain(
      'Try reducing the document size'
    );
    expect(h.getSuggestions(new PuppeteerError('crash'))[0]).toContain('Reinstall Puppeteer');
  });

  test('handleError logs to console when no logger provided', async () => {
    const h = new ErrorHandler();
    const logs: unknown[][] = [];
    jest.spyOn(console, 'error').mockImplementation((...args: unknown[]) => logs.push(args));
